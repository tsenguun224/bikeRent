...
exports.login = asyncHandler(async (req, res, next) => {
	...
	res.status(200).cookie("bike-token", token, cookieOption).json({
		success: true,
		token,
		user: user
	});
...
exports.logout = asyncHandler(async (req, res, next) => {
	...
	res.status(200).cookie("bike-token", null, cookieOption).json({
		success: true,
		data: "logged out..."
	});
});
...
exports.manageBalance = asyncHandler(async (req, res, next) => {
	...
	if (user) {
		if (transaction.type === "exp") {
			const updatedBal = await User.findByIdAndUpdate(user._id, {
